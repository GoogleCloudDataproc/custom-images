...
function run_install_optional_components_script() {
  ...
  cat > ./install_optional_components.sh <<EOF
...
USER_DATAPROC_COMPONENTS=(${USER_DATAPROC_COMPONENTS})
source "${BDUTIL_DIR}/install_optional_components.sh"
EOF

  # run custom image creation script
  bash -x ./install_optional_components.sh
